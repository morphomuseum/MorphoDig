...
mqMorphoDigCore::mqMorphoDigCore()
{
	...
	this->mui_MaskInside = 1; // masks inside
	this->mui_PencilMaskSphere = 1; // pencil: spherical shape by default
	this->mui_MaskOn = 1; // mask (and not erase mask =unmask)
...
void mqMorphoDigCore::MaskWithSurface(vtkMDVolume *myVolume, vtkMDActor *myActor)
{
	if (myVolume != NULL && myVolume->GetMaskEnabled() == 1)
	{
		...
		if (myActor != NULL)
		{
			...
			for (vtkIdType j = 0; j < surface->GetNumberOfPoints(); j++) {
				// for every triangle 
				surface->GetPoint(j, ve_init_pos);
			...
			// 2 use the same scheme as int MaskAt
			...
			int pencilSphere = this->Getmui_PencilMaskSphere();
...
void mqMorphoDigCore::OpenRawVolume(QString fileName, QString objectName, int dataType, int dimX, int dimY, int dimZ, double headerSize, double voxelSizeX, double voxelSizeY, double voxelSizeZ, bool bigEndian, bool frontToBack)
{
	int file_exists = 1;
	QFile file(fileName);
	QString name = "";
	...
	if (file_exists == 1)
	{
		...
		vtkSmartPointer <vtkImageReader> rawReader = vtkSmartPointer<vtkImageReader>::New();
			rawReader->SetFileName(fileName.toLocal8Bit());
			if (bigEndian)
			{
...
void mqMorphoDigCore::OpenDicomFolder(QString folderName)
{
	...
	reader->SetDirectoryName(folderName.toLocal8Bit());
...
void mqMorphoDigCore::OpenTiff3DVolume(QString fileName, QString objectName, double voxelSizeX, double voxelSizeY, double voxelSizeZ, bool frontToBack)
{
	int file_exists = 1;
	QFile file(fileName);
	QString name = "";
...
int mqMorphoDigCore::OpenMesh(QString fileName)
{
	...
	if (file_exists == 1)
	{
		...
		std::cout << "Open PLY: " <<type<< std::endl;
		...
		{
			std::cout << "Create PLY reader: " << type << std::endl;
			...
			reader->Update();
			MyPolyData = reader->GetOutput();
			std::cout << "Got an output from PLY reader "<< std::endl;
		...
		std::cout << "\nClean polydata filter done" <<  std::endl;
		

		//Surface can contain a few "non-manifold" edges
		//ANYWAY IT WAS NOT USED AND MADE MORPHODIG CRASH FOR ANNE-LISE!
		...
		std::cout << "NON MANIFOLD pts:" << num_nonmanifold_pts << " , cells:" << num_nonmanifold_cells << std::endl;
		*/
		MyPolyData = cleanPolyDataFilter->GetOutput();
		std::cout << "\nNumber of points after cleaning!:" << MyPolyData->GetNumberOfPoints() << std::endl;
		std::cout << "\nNumber of cells after cleaning!:" << MyPolyData->GetNumberOfCells() << std::endl;
...
/*CutSurfaceAlongCUR*/
int mqMorphoDigCore::CutSurfaceAlongCUR(QString fileName, int default_decimation, int save_format)
{
	//Pour le moment, ça ne doit fonctionner que si on a 1 seul segment... ou bien on ne le fait que pour le premier segment ?
	int save_other_lmks = 0;
	int milestones_once = 1;
	std::string STVext = ".stv";
	std::string STVext2 = ".STV";
	std::string VTPext = ".vtp";
	std::string VTPext2 = ".VTP";
	...
	//Combien de morceaux coupés en tout
	// combien de segments:

	int tot_num_seg = this->GetNumberOfCurveSegments();
	int tot_cut_number = 0;
	tot_cut_number += this->Getmui_SegmentDecimations()->decimation.at(0);
	// on ne fera que le 1er segment. 
	
	cout << "tot_cut_number=" << tot_cut_number<< endl;

	vtkIdType ci = 0;
	std::string csi = "00";
	std::string lsi = "00";
	//MODE TEXTE UNIQUEMENT POUR LE FORMAT STV
	//=> SINON ON FAIT vtkAppendPolyData
	QFile file(fileName);

	if (save_format == 0)//STV
	{
		if (file.open(QIODevice::WriteOnly | QIODevice::Text))
		{
			QTextStream stream(&file);
			...
			double  nn1[3], nn2[3], hh1[3], hh2[3], intvv[3], intvv2[3], slm[3], norm[3];
					...
					if (ob_NB != NULL && ob_HB != NULL)
					{
						...
						if (compute == 1)
						{
							...
							for (m = 0; m <= nint; m++)
							{
								...
								norm[2] = intvv2[2] - intvv[2];
								norm[1] = intvv2[1] - intvv[1];
								norm[0] = intvv2[0] - intvv[0];
								...
								if ((current_length >= length_to_reach) && (length_to_reach >= preceding_length) && (decimation_index < decimation))
								{
									...
									{
										// C'EST ICI QU'ON A LE PLAN.... IL FAUT AVOIR LA NORMALE AUSSI
										if (save_format == 0)
										{
											norm[0] += slm[0];
											norm[1] += slm[1];
											...
											// => ICI J'AI DE QUOI INSTANCIER VTKCUTTER! Le barycentre du plan SLM et la nomale NORM! 
										}
							...
							if ((current_length >= length_to_reach) && (length_to_reach >= preceding_length) && (decimation_index < decimation))
							{
								...
								if (forget_milestone_start == 0) //99.99% of the cases
								{
									if (save_format == 0)
									{
										...
										norm[1] += slm[1];
										norm[2] += slm[2];
										stream << "Curve_segment:" << num_seg << "-" << decimation_index << " " << slm[0] << " " << slm[1] << " " << slm[2] <<  " "<<norm[0]<< " " << norm[1] << " " << norm[2] <<  Qt::endl;
...
int mqMorphoDigCore::Getmui_PencilMaskSphere() { return this->mui_PencilMaskSphere; }
void mqMorphoDigCore::Setmui_PencilMaskSphere(int sphere) { this->mui_PencilMaskSphere = sphere; }
	int mqMorphoDigCore::Getmui_MaskInside() { return this->mui_MaskInside; }
	void mqMorphoDigCore::Setmui_MaskInside(int inside) { this->mui_MaskInside = inside; }
	int mqMorphoDigCore::Getmui_MaskOn() { return this->mui_MaskOn; }
	void mqMorphoDigCore::Setmui_MaskOn(int maskOn) { this->mui_MaskOn = maskOn; }
